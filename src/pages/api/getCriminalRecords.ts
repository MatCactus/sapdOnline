...
import { GetCriminalRecordsRes } from './types';
...
export default async function handler(
    ...
    let dbRes: GetCriminalRecordsRes = await DBConnect(`SELECT id, criminalrecords.owner_name, criminalrecords.owner_surname, records, users.name, users.surname, users.pon, docreation FROM criminalrecords INNER JOIN users ON criminalrecords.author = users.username`).catch((e) => {
    ...
    dbRes.forEach((record: any) => {
        record.records = JSON.parse(record.records).length;
    });
